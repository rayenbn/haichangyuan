    ...
    <div class="container padding-top-120 padding-bottom-120">
        ...
        <div class="row gallery-wrapper grid">
            @foreach ($galleries as $image)
                ...
                @endphp
            <div class="col-lg-4 col-md-6 col-12 {{ $types }} grid-item">
                <div class="gallery-img">
                    <img src="/storage/gallery/thumbnail/{{ $image->picture ?? ''}}" alt="">
                    <div class="gallery-info">
                        <div class="inner-info text-left">
                           {{--<p>{!! $image->type !!}</p>--}} 
                            <h6>{!! $image->title !!}</h6>
                            <br>
                            @if ($image->link)
                            <a href="{{ $image->link ?? ''}}" style="color: white; font-size: 24px;"><span><i class="fas fa-arrow-right"></i></span></a>
                            @endif
                        </div>
                    </div>
                </div>
